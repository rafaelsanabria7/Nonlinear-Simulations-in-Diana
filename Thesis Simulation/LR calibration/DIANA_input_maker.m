% input data (.DAT) for DIANA and Machine learning
% LR punching shear slab 
% Rafael sanabria 2020
...
specific_lines = [ 20232 20237 20238 20242 20243 20251];
...
% capture specific lines
specific_output = [40 62 143 165 241 263];

% replacing output names
B{specific_output(1)} = cell2mat(strcat({'    FILE  LR_'},num2str(j),'_FORCE'));
B{specific_output(2)} = cell2mat(strcat({'    FILE  LR_'},num2str(j),'_DISP'));
B{specific_output(3)} = cell2mat(strcat({'    FILE  LR_'},num2str(j),'_FORCE'));
B{specific_output(4)} = cell2mat(strcat({'    FILE  LR_'},num2str(j),'_DISP'));
B{specific_output(5)} = cell2mat(strcat({'    FILE  LR_'},num2str(j),'_FORCE'));
B{specific_output(6)} = cell2mat(strcat({'    FILE  LR_'},num2str(j),'_DISP'));
...
% Load step
k = 0;
model_path = 'C:/Users/labmem/Desktop/Rafael/LR Machine Learning/';
for j=1:30
